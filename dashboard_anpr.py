# ============================================================
# IBVAP - ANPR Integration for Dashboard (FIXED)
# ============================================================
...
try:
    ...
    print("⚠️ EasyOCR not installed. ANPR disabled.")
...
print("=" * 70)
print("🛡️ IBVAP - GPU Mode with ANPR & LLM")
print("=" * 70)
...
stats = {
    ...
    'anpr_status': 'Active' if anpr_reader is not None else 'Disabled',
    'plates_detected': 0
}
processing = True
...
def detect_plates(frame):
    ...
    try:
        ...
        for (bbox, text, confidence) in results:
            # Filter for plate-like text (alphanumeric, length 4-10)
...
def generate_frames():
    ...
    while processing:
        ...
        try:
            ...
            # ANPR Detection on vehicles (every 10 frames)
            if anpr_reader is not None and frame_count % 10 == 0 and vehicle_count > 0:
                plates = detect_plates(frame)
                for plate in plates:
                    plates_detected.append(plate['text'])
                    stats['plates_detected'] += 1
                    alert_msg = f"📋 License Plate Detected: {plate['text']} (Conf: {plate['confidence']:.2f})"
            ...
            # Draw overlays
            cv2.putText(annotated, f"People: {people_count} | Vehicles: {vehicle_count} | Plates: {stats['plates_detected']}", 
                       (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
            
            cv2.putText(annotated, f"Detections: {det_count} | FPS: {stats['fps']} | ANPR: {stats['anpr_status']}", 
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .badge.anpr { color: #ffd93d; border-color: #ffd93d; }
        .status-dot {
        ...
        .alert-item.anpr { border-left-color: #ffd93d; }
...
<body>
    <div class="dashboard">
        <div class="header">
            <h1>🛡️ IBVAP <span style="font-size:14px;color:#667788;">ANPR + LLM</span></h1>
            <div>
                <span class="badge gpu">⚡ GPU</span>
                <span class="badge anpr">📋 ANPR</span>
    ...
    <script>
        ...
        socket.on('new_alert', function(data) {
            ...
            var isAnpr = data.message.includes('License Plate');
            alertDiv.className = 'alert-item' + (isAnpr ? ' anpr' : '');
...
if __name__ == '__main__':
    print("=" * 70)
    print("🛡️ IBVAP - GPU + ANPR + LLM")
    print("=" * 70)
    print(f"🌐 Dashboard: http://localhost:5000")
    print(f"🤖 GPU: {torch.cuda.get_device_name(0) if torch.cuda.is_available() else 'None'}")
    print(f"📋 ANPR: {'Available' if anpr_reader is not None else 'Disabled'}")
