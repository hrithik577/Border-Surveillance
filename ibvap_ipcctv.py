...
RTSP_URL = "rtsp://10.87.96.21:8554/video_main"
...
class IBVAP:
    ...
    def run(self):
        # Try to connect to RTSP stream with quality settings
        ...
        if not cap.isOpened():
            print(f"❌ Could not connect to RTSP stream: {RTSP_URL}")
            print("⚠️ Make sure IPyCam is running!")
            return

        print("✅ Connected to IP CCTV via RTSP")
        ...
        cv2.namedWindow('IBVAP - IP CCTV Mode', cv2.WINDOW_NORMAL)
        cv2.resizeWindow('IBVAP - IP CCTV Mode', 1920, 1080)
        ...
        while True:
            ...
            cv2.imshow('IBVAP - IP CCTV Mode', annotated_frame)
