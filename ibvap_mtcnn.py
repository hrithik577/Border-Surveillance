...
class IBVAP:
    def __init__(self, rtsp_url, model_path='models/yolov8n.pt'):
        ...
        print(f"✅ IBVAP with Face Detection (MTCNN)")
    ...
    def run(self):
        # IMPROVED: Higher quality settings
        cap = cv2.VideoCapture(self.rtsp_url, cv2.CAP_FFMPEG)
        
        # Increase buffer for smoother playback
        cap.set(cv2.CAP_PROP_BUFFERSIZE, 10)
        
        # Try to get actual video properties
        ...
        cv2.namedWindow('IBVAP - Border Surveillance + Face Detection', cv2.WINDOW_NORMAL)
        cv2.resizeWindow('IBVAP - Border Surveillance + Face Detection', 1920, 1080)
        ...
        while True:
            ...
            # Only resize if frame is too large for processing
            h, w = frame.shape[:2]
            if w > 1920 or h > 1080:
            ...
            cv2.imshow('IBVAP - Border Surveillance + Face Detection', annotated_frame)
